...
from agent import build_react_agent, run_agent, load_spec, table, s3, BUCKET_NAME, PENDING_CLEANUP_PREFIX
import verify
...
def capture_state():
    ...
    for page in paginator.paginate(Bucket=BUCKET_NAME):
        for obj in page.get("Contents", []):
            all_objects.append({"key": obj["Key"], "size": obj["Size"]})

    active_objects = [o for o in all_objects if not o["key"].startswith(PENDING_CLEANUP_PREFIX)]
    pending_objects = [o for o in all_objects if o["key"].startswith(PENDING_CLEANUP_PREFIX)]

    scheduled_jobs = table.scan().get("Items", [])
...
def run_live(record=False, output_path=None, label=None):
    ...
    if record:
        fixture = {
            "demo": "04_literal_interpretation",
            "mode": label,
            "spec": spec_text,
            "steps": steps,
