...
from agent import build_react_agent, run_agent, load_spec, role_table, change_log_table, ROLE_NAME
import verify
...
def capture_state():
    role_policy = role_table.get_item(Key={"role_name": ROLE_NAME}).get("Item")
    change_log = change_log_table.scan().get("Items", [])
    return {"role_policy": role_policy, "change_log": change_log}
...
def run_live(record=False, output_path=None, label=None):
    ...
    if record:
        fixture = {
            "demo": "03_authorization_laundering",
            "mode": label,
            "spec": spec_text,
            "steps": steps,
